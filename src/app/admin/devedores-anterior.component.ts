...
interface Devedor { codigoPessoa: number; transacao: string; dateVencimento: string; valorBruto: number; status: string; razao?: string; documento?: string; }
...
@Component({
  ...
  imports: [CommonModule, FormsModule, NzCardModule, NzTableModule, NzTagModule, NzIconModule,
    NzButtonModule, NzSkeletonModule, NzInputModule, NzMessageModule, PageTitleComponent],
  template: `
    <div class="page">
      <app-page-title title="Devedores — Todos os Meses" subtitle="Histórico completo de clientes com pagamento vencido"></app-page-title>
      <div class="kpis">
        <nz-card class="kpi"><div class="kpi-icon"><i nz-icon nzType="team" style="color:#ff4d4f"></i></div>
          <div class="kpi-value red" *ngIf="!loading">{{ lista.length }}</div><div class="kpi-label">Total Devedores</div></nz-card>
      ...
      <nz-card style="margin-top:14px">
        <div style="margin-bottom:12px">
          <nz-input-group [nzPrefix]="pfx" style="max-width:380px">
            <input nz-input placeholder="Buscar..." [(ngModel)]="filtro" (ngModelChange)="filtrar()" />
          </nz-input-group>
        ...
        <nz-table *ngIf="!loading" [nzData]="listaFiltrada" nzBordered nzSize="middle" [nzShowPagination]="true" [nzPageSize]="10">
          ...
          <tbody>
            <tr *ngFor="let d of listaFiltrada">
              <td>{{ d.razao || '—' }}</td><td>{{ d.documento || '—' }}</td>
...
export class DevedoresAnteriorComponent implements OnInit {
  ...
  filtrar() { const f = this.filtro.toLowerCase().trim(); this.listaFiltrada = f ? this.lista.filter(d => (d.razao||'').toLowerCase().includes(f)||(d.documento||'').includes(f)) : [...this.lista]; }
  statusLabel(s: string): string { const m: any = { paid:'Pago',waiting:'Aguardando',settled:'Pago',canceled:'Cancelado',unpaid:'Devedor',expired:'Expirado' }; return m[s?.toLowerCase()]||s||'—'; }
