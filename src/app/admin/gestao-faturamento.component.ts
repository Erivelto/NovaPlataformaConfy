...
@Component({
  selector: 'app-gestao-faturamento',
  standalone: true,
  ...
  template: `
    <div class="gestao-faturamento">
      <app-page-title title="Gestão Faturamento" subtitle="Lista de clientes para faturamento">
        <app-export-excel-button
          [data]="$any(clientesFiltrados)"
          [columns]="exportColumns"
          fileName="gestao-faturamento"
          [loading]="loading" />
      </app-page-title>
      ...
      <nz-card style="margin-top:14px">
        <div class="toolbar">
          ...
          <nz-select
            ...
            [(ngModel)]="filtroTipo"
            (ngModelChange)="filtrar()"
            nzPlaceHolder="Física ou Online">
          ...
          <nz-select
            class="filtro-select filtro-prefeitura"
            [(ngModel)]="filtroPrefeitura"
            (ngModelChange)="filtrar()"
            nzShowSearch
            nzAllowClear
            nzPlaceHolder="Prefeitura">
          ...
          *ngIf="!loading"
          ...
          <thead>
            <tr>
              ...
              <th nzWidth="100px" nzAlign="center">Ações</th>
            </tr>
          </thead>
          <tbody>
  ...
  `,
  styles: [`
    .gestao-faturamento { padding: 8px 4px; }
    ...
    .filtro-prefeitura { min-width: 220px; width: auto; }
...
export class GestaoFaturamentoComponent implements OnInit {
  ...
  detalhes(c: Pessoa): void {
    this.router.navigate(['/administrativo/gestao-faturamento/detalhe', c.codigo], {
      queryParams: { tipo: c.fisica ? 'fisica' : 'online' }
    });
  }
}
